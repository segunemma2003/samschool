...
Route::middleware([
    ...
    Route::get('/student/result/download/{studentId}/{termId}/{academicYearId}', function ($studentId, $termId, $academicYearId) {
        ...
        try {
            ...
            // Return PDF directly (temporarily without S3)
            return $pdf->stream($filename);

            // TODO: Re-enable S3 storage when configured properly
            // $tenantId = tenant('id') ?? 'default';
            // $s3Filename = "results/{$tenantId}/{$filename}";
            // \Illuminate\Support\Facades\Storage::disk('s3')->put($s3Filename, $pdf->output());
            // return redirect(\Illuminate\Support\Facades\Storage::disk('s3')->url($s3Filename));
